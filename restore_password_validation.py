...
class Restore_Password_Validation:
    ...
    def __init__(self):
        ...
        # head lbl
        restore_head_lbl = Label(restore_root, text="Restore pasword", font=("plain", 30))
        restore_head_lbl.pack(pady=10)

        restore_head_lbl2 = Label(restore_root, text="We will send a validation code to your email to restore your email", font=("plain", 10))
        restore_head_lbl2.pack(pady=10)        

        # restore email & password entry
        restore_email_lbl = Label(restore_root, text="Email", font="plain")
        ...
        check_btn = Checkbutton(restore_root, text="I'am not robot", cursor="hand2", command=check_btn_func, activeforeground="red")
